...
int main(void) {
    ...
    */
    ...
    while (1) {
        if (turn_enable == 0) {
            turn_enable_counter += SENSOR_READ_DELAY_MS;
            if (turn_enable_counter >= FIVE_SECONDS) {
                turn_enable = 1;
        ...
        if (turn_enable && (sensor_delta > DELTA_THRESHOLD) ) {
            // do a large turn to account for large delta between sensors
            // after turning, wait 5 seconds before enabling another turn
            turn_enable = 0;
