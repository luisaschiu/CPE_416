...
#define SQUARE_UP_SPEED_POS 20
#define SQUARE_UP_SPEED_NEG -18
...
void push_can_off_edge() {
    ...
    while (right_sensor < EDGE_SENSOR_THRESHOLD){
        motor(LEFT_MOTOR, SQUARE_UP_SPEED_NEG);
        motor(RIGHT_MOTOR, SQUARE_UP_SPEED_POS);
        left_sensor = analog(LEFT_LINE_SENSOR);
        right_sensor = analog(RIGHT_LINE_SENSOR);
        clear_screen();
        lcd_cursor(5,0);
    ...
    while (left_sensor < EDGE_SENSOR_THRESHOLD){
        motor(LEFT_MOTOR, SQUARE_UP_SPEED_POS);
        motor(RIGHT_MOTOR, SQUARE_UP_SPEED_NEG);
        left_sensor = analog(LEFT_LINE_SENSOR);
        right_sensor = analog(RIGHT_LINE_SENSOR);
        clear_screen();
        lcd_cursor(5,0);
