...
//    print_particle_array_everything(sum_weight_array);
    ...
    for (uint8_t j = 0; j < NUM_PARTICLES; j++){
        uint8_t random = roundf(( (float) rand() ) / ( (float) RAND_MAX )*j);
...
float calculate_std_dev(uint8_t size, struct particle particle_array[]) {
    // printf("ENTERED calculate_std_dev\n");
    struct particle std_dev_array[NUM_PARTICLES];
    memcpy(std_dev_array, particle_array, sizeof(struct particle)*NUM_PARTICLES);
    ...
    */
    ...
    for (uint8_t i = 0; i < NUM_PARTICLES; i++) {
        if (std_dev_array[i].angle <= 0 + ANGLE_THRESHOLD) {
            num_particles_near_0++;
        }
        if (std_dev_array[i].angle >= 360 - ANGLE_THRESHOLD) {
    ...
    if (num_particles_near_0 + num_particles_near_360 >= particle_threshold) {
        ...
        if (num_particles_near_0 > num_particles_near_360) {
            for (uint8_t i = 0; i < NUM_PARTICLES; i++) {
                if (std_dev_array[i].angle >= 360 - ANGLE_THRESHOLD) {
                    std_dev_array[i].angle = 360 - std_dev_array[i].angle;
                }
            }
        }
