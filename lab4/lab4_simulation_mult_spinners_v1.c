...
int main(void) {
    ...
    */
    ...
    float std_dev;
    uint8_t adv_particle_counter = 0;
    /**
    ...
    */
    while(1) {
        ...
        for (uint8_t i = 0; i < NUM_PARTICLES; i ++){
            ...
            // printf("Press ENTER key to Print Values. After classifying, before calculating probability.\n");  
            // getchar();
            ...
            // printf("Press ENTER key to Print Values. After classifying, after passing through probability.\n");  
            // getchar();
            // print_particle_array_everything(particle_array);
        }
        ...
        // printf("Press ENTER key to Print Values. After normalizing. Before selection sorting.\n");  
        // getchar();
        // print_particle_array_everything(particle_array);

//        selection_sort(particle_array, NUM_PARTICLES);

        printf("Press ENTER key to Print Values normalized, before resampling.\n");  
        getchar();
        print_particle_array_everything(particle_array);
        // printf("Press ENTER key to Continue Resampling.\n");  
        // getchar();

        resample_particles(particle_array);

        printf("Resampled. Press ENTER key to Continue\n");  
        getchar();
        print_particle_array_everything(particle_array);
    ...
    for (uint8_t i = 0; i < NUM_PARTICLES; i++) {
        if (particle_array[i].weight != 0.00){
            printf("%s", "Angles: ");
            printf("%.6f ", particle_array[i].angle); 
            printf("\n"); 
        }
    }
    printf("%d", adv_particle_counter); 
    printf("\n"); 
}
...
//    print_particle_array_everything(particle_array);
    ...
    struct particle sum_weight_array[NUM_PARTICLES]; //new
    struct particle resampled_particle_array[NUM_PARTICLES]; //new
...
//    print_particle_array_everything(sum_weight_array);
    ...
    for (uint8_t j = 0; j < NUM_PARTICLES; j++){
        float U = particle_array[j].weight + j*1/NUM_PARTICLES;
    ...
    for (uint8_t i=0.05*NUM_PARTICLES; i < NUM_PARTICLES; i++) {
    ...
    printf("Press ENTER key to Print resampled array after noise.\n");  
    getchar();
    print_particle_array_everything(resampled_particle_array);

    // re-assign weights to new particle_array
        for (uint8_t j = 0; j < NUM_PARTICLES; j++){
            particle_array[i].weight = resampled_particle_array[j].weight;
            particle_array[i].angle = resampled_particle_array[j].angle;
            particle_array[i].classification = resampled_particle_array[j].classification;
    }
}
...
float box_muller_transform() {
    ...
    float co = (float) cos(2.0*3.14159265359*u2); // NOTE: REPLACE WITH M_PI ONCE MATH.H WORKS AGAIN.
    float z = sq*co;
    return z;
}
