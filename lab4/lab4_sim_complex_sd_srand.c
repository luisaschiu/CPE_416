 ...
 * Assignment Lab 4 Simulation
 * Description: Line following proportional controller localizes itself 
 * by reading range sensor values. Placed in a random location on a circular 
 * track, the robot speculates where it is by assigning weights to particles
 * and resampling based on weights until the particles gather in one particular
 * location.
*/
...
int main() {
    ...
    while(1) {
        ...
        */
        ...
        printf("NORMALIZED. Press ENTER key to Continue\n");  
        // getchar();
        /* resample particles */
...
uint8_t get_simulated_sensor_value(float position, uint8_t num_tower, uint16_t tower_loc[]) {
    ...
    for (uint8_t i = 0; i < num_tower; i++){
        if (min_diff_between_angles(position, (float) tower_loc[i]) < LOCATION_TOLERANCE) { // tower
            simulated_sensor_value = (c_tower + b_tower) / 2;
            return simulated_sensor_value;
        }
        else { // free space
            simulated_sensor_value = (c_free + b_free) / 2;
        }
    }
...
/**
 * add in 5% random particles
*/
void resample(struct particle particle_array[]) {
